...
namespace ManagedCuda.CudaDNN
{
	...
	public class FilterDescriptor : IDisposable
	{
		...
		public FilterDescriptor()
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreateFilterDescriptor", res));
		...
		protected virtual void Dispose(bool fDisposing)
		{
			if (fDisposing && !disposed)
			{
				...
				Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyFilterDescriptor", res));
				disposed = true;
			}
			if (!fDisposing && !disposed)
											...
											)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetFilter4dDescriptor", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
