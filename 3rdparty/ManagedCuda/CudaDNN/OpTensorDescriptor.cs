...
namespace ManagedCuda.CudaDNN
{
    ...
    public class OpTensorDescriptor : IDisposable
    {
        ...
        public OpTensorDescriptor(CudaDNNContext context)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreateOpTensorDescriptor", res));
        ...
        protected virtual void Dispose(bool fDisposing)
        {
            if (fDisposing && !disposed)
            {
                ...
                Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyOpTensorDescriptor", res));
                disposed = true;
            }
            if (!fDisposing && !disposed)
                                        ...
                                        cudnnNanPropagation opTensorNanOpt)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetOpTensorDescriptor", res));
            if (res != cudnnStatus.Success) throw new CudaDNNException(res);
        }
                                        ...
                                        ref cudnnNanPropagation opTensorNanOpt)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnGetOpTensorDescriptor", res));
            if (res != cudnnStatus.Success) throw new CudaDNNException(res);
        }
    }
}
