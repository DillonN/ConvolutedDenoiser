...
namespace ManagedCuda.CudaDNN
{
	...
	public class LRNDescriptor : IDisposable
	{
		...
		public LRNDescriptor(CudaDNNContext context)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreateLRNDescriptor", res));
		...
		protected virtual void Dispose(bool fDisposing)
		{
			if (fDisposing && !disposed)
			{
				...
				Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyLRNDescriptor", res));
				disposed = true;
			}
			if (!fDisposing && !disposed)
												...
												)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetLRNDescriptor", res));
												...
												)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnGetLRNDescriptor", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
