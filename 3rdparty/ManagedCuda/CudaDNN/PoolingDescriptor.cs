...
namespace ManagedCuda.CudaDNN
{
	...
	public class PoolingDescriptor : IDisposable
	{
		...
		public PoolingDescriptor()
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreatePoolingDescriptor", res));
		...
		protected virtual void Dispose(bool fDisposing)
		{
			if (fDisposing && !disposed)
			{
				...
				Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyPoolingDescriptor", res));
				disposed = true;
			}
			if (!fDisposing && !disposed)
														   ...
														   )
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetPooling2dDescriptor", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
										...
										)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnGetPooling2dDescriptor", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
										...
										)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetPoolingNdDescriptor", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
											...
											)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnGetPoolingNdDescriptor", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
																	 ...
																	 int[] outputTensorDimA)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnGetPoolingNdForwardOutputDim", res));
			if (res != cudnnStatus.Success) throw new CudaDNNException(res);
		}
