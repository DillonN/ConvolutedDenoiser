...
namespace ManagedCuda.CudaDNN
{
    ...
    public class CTCLossDescriptor : IDisposable
    {
        ...
        public CTCLossDescriptor(CudaDNNContext context)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreateCTCLossDescriptor", res));
        ...
        protected virtual void Dispose(bool fDisposing)
        {
            if (fDisposing && !disposed)
            {
                ...
                Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyCTCLossDescriptor", res));
                disposed = true;
            }
            if (!fDisposing && !disposed)
