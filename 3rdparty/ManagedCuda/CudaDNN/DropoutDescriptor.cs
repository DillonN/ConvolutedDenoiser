...
namespace ManagedCuda.CudaDNN
{
    ...
    public class DropoutDescriptor : IDisposable
    {
        ...
        public DropoutDescriptor(CudaDNNContext context)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreateDropoutDescriptor", res));
        ...
        protected virtual void Dispose(bool fDisposing)
        {
            if (fDisposing && !disposed)
            {
                ...
                Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyDropoutDescriptor", res));
                disposed = true;
            }
            if (!fDisposing && !disposed)
                                                            ...
                                                            ulong seed)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetDropoutDescriptor", res));
            if (res != cudnnStatus.Success) throw new CudaDNNException(res);
        }
    }
}
