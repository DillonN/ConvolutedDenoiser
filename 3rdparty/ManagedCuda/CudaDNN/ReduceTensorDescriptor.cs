...
namespace ManagedCuda.CudaDNN
{
    ...
    public class ReduceTensorDescriptor : IDisposable
    {
        ...
        public ReduceTensorDescriptor()
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnCreateReduceTensorDescriptor", res));
        ...
        protected virtual void Dispose(bool fDisposing)
        {
            if (fDisposing && !disposed)
            {
                ...
                Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnDestroyReduceTensorDescriptor", res));
                disposed = true;
            }
            if (!fDisposing && !disposed)
                                        ...
                                        cudnnIndicesType reduceTensorIndicesType)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cudnnSetReduceTensorDescriptor", res));
            if (res != cudnnStatus.Success) throw new CudaDNNException(res);
        }
