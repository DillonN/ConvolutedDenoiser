...
namespace ManagedCuda
{
    ...
    public class CudaDirectXInteropResource : ICudaGraphicsInteropResource
    {
        ...
        public CudaDirectXInteropResource(IntPtr d3dResource, CUGraphicsRegisterFlags flags, CudaContext.DirectXVersion version)
        {
            ...
            switch (version)
            {
                case CudaContext.DirectXVersion.D3D9:
                    ...
                    Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuGraphicsD3D9RegisterResource", res));
                    break;
                case CudaContext.DirectXVersion.D3D10:
                    ...
                    Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuGraphicsD3D10RegisterResource", res));
                    break;
                case CudaContext.DirectXVersion.D3D11:
                    ...
                    Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuGraphicsD3D11RegisterResource", res));
                    break;
                default:
        ...
        protected virtual void Dispose(bool fDisposing)
        {
            if (fDisposing && !disposed)
            {
                ...
                if (_IsRegistered)
                {
                    ...
                    Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuGraphicsUnregisterResource", res));
                }
                disposed = true;
            }
            if (!fDisposing && !disposed)
        ...
        public void Map(CUstream stream)
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuGraphicsMapResources", res));
        ...
        public CudaDeviceVariable<T> GetMappedPointer<T>() where T : struct
        {
            ...
            Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuGraphicsResourceGetMappedPointer", res));
            if (res != CUResult.Success)
                throw new CudaException(res);
