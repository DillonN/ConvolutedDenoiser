...
namespace ManagedCuda
{
	...
	public class CudaMipmappedArray : IDisposable
	{
		...
		public CudaMipmappedArray(CUDAArray3DDescriptor descriptor, uint numMipmapLevels)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuMipmappedArrayCreate", res));
		...
		protected virtual void Dispose(bool fDisposing)
		{
			if (fDisposing && !disposed)
			{
				if (_isOwner)
				{
					...
					Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuMipmappedArrayDestroy", res));
				}
				disposed = true;
			}
			if (!fDisposing && !disposed)
		...
		public CudaArray1D GetLevelAsArray1D(uint level)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuMipmappedArrayGetLevel", res));
