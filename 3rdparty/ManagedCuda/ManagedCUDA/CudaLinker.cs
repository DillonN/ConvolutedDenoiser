	...
	public class CudaLinker : IDisposable
	{
		...
		protected virtual void Dispose(bool fDisposing)
		{
			...
			{	
				...
				Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuLinkDestroy", res));
				disposed = true;
			}
			if (!fDisposing && !disposed)
		...
		public void AddData(byte[] data, CUJITInputType type, string name, CudaJitOptionCollection options)
		{
			if (options == null)
			{
				...
				Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuLinkAddData", res));
				if (res != CUResult.Success)
					throw new CudaException(res);
			}
			else
		...
		public byte[] Complete()
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuLinkComplete", res));
			if (res != CUResult.Success)
				throw new CudaException(res);
