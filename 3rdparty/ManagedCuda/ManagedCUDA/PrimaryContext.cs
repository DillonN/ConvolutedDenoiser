	...
	public class PrimaryContext : CudaContext
	{
		...
		protected override void Dispose(bool fDisposing)
		{
			if (fDisposing && !disposed)
			{
				...
				Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuDevicePrimaryCtxRelease", res));
		...
		public static void SetFlags(CUdevice device, CUCtxFlags flags)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuDevicePrimaryCtxSetFlags", res));
			if (res != CUResult.Success)
				throw new CudaException(res);
		}
		...
		public static void GetState(CUdevice device, out CUCtxFlags flags, out bool active)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuDevicePrimaryCtxGetState", res));
			if (res != CUResult.Success)
				throw new CudaException(res);
			active = temp == 1;
		}
		...
		public static void Reset(CUdevice device)
		{
			...
			Debug.Write("");//Line(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuDevicePrimaryCtxReset", res));
			if (res != CUResult.Success)
				throw new CudaException(res);
		}
		#endregion
